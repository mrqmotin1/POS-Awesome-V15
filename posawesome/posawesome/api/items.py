...
def get_last_invoice_rates(customer, item_codes, company=None):
    """Return the last invoiced rate for each item for the given customer.

    Args:
        customer (str): Customer identifier.
        item_codes (list[str] | str): List of item codes or JSON string.
        company (str | None): Optional company filter.

    Returns:
        dict: Mapping of item_code -> {"rate": float, "currency": str}
    """
    ...
    normalized_codes = [code for code in item_codes if code]
        ...
        FROM (
            SELECT
                ...
                ROW_NUMBER() OVER (
                    ...
                    ORDER BY si.posting_date DESC, si.posting_time DESC, sii.creation DESC
                ) AS rn
    ...
    rows = frappe.db.sql(query, params, as_dict=True) or []
    return {row.item_code: {"rate": flt(row.rate), "currency": row.currency} for row in rows}
