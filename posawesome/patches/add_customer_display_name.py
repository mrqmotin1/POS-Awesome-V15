...
def execute():
    ...
    row = frappe.db.get_value(
        "Custom Field", CUSTOM_FIELD_NAME, ["fieldtype", "options"], as_dict=True
    )

    if row and row.fieldtype == "Select" and not row.options:
        ...
        print(
            f"FAILED to set up POS Profile field '{FIELDNAME}': fieldtype is "
            f"'{row.fieldtype}' with options {row.options!r}, expected 'Select' with none. "
