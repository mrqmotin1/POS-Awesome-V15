...
describe("customerDisplay utils", () => {
	...
	it("carries the ready handshake in both directions", () => {
		const publisher = createCustomerDisplayTransport("cd_ready");
		const display = createCustomerDisplayTransport("cd_ready");

		const seenByPos: string[] = [];
		const seenByDisplay: string[] = [];
		const stopPos = publisher.subscribeControl((action) => {
			seenByPos.push(action);
			if (action === "display_ready") {
				publisher.publishControl("enter_fullscreen");
			}
		});
		const stopDisplay = display.subscribeControl((action) =>
			seenByDisplay.push(action),
		);

		display.publishControl("display_ready");

		// BroadcastChannel never echoes to the sender.
		expect(seenByPos).toEqual(["display_ready"]);
		expect(seenByDisplay).toEqual(["enter_fullscreen"]);
