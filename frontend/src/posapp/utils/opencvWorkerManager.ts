...
class OpenCVWorkerManager {
    ...
    async processImage(imageData: ImageData, options: ProcessOptions = {}): Promise<any> {
        if (!this.initialized) {
    ...
    async detectBarcodes(imageData: ImageData, options: ProcessOptions = {}): Promise<any> {
        if (!this.initialized) {
            await this.initialize();
        }
    ...
    private _handleWorkerMessage({ id, type, data, error }: WorkerMessage): void {
