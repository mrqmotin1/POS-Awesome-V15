 ...
 */
export type StockListener = (event: StockChangeEvent) => void;
...
const notifyListeners = (type: string, codes: string[], meta: Record<string, any> = {}): void => {
...
export const updateBaseQuantities = (entries: StockEntry[] = [], options: StockUpdateOptions = {}): string[] => {
...
export const updateReservations = (totals: Record<string, any> = {}, options: StockUpdateOptions = {}): string[] => {
...
export const clearAll = (): string[] => {
    const affected = Array.from(
        new Set([...baseQuantities.keys(), ...reservedQuantities.keys(), ...availabilityMap.keys()])
    );
...
export const getSnapshot = (codes?: string[] | string): AvailabilitySnapshot => {
    if (!codes) {
...
export const applyAvailabilityToItem = (item: any, options: StockUpdateOptions = {}): void => {
...
export const applyAvailabilityToCollection = (items: any[], codesSet: Set<string> | null = null, options: StockUpdateOptions = {}): void => {
...
export const primeFromItems = (items: any[] = [], options: StockUpdateOptions = {}): string[] => {
...
export const applyInvoiceConsumption = (items: any[] = [], options: StockUpdateOptions = {}): string[] => {
    const { silent = false } = options;
    const changed = new Set<string>();
    items.forEach((entry) => {
        if (!entry) {
