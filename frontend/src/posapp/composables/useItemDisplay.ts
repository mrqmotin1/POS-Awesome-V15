 ...
 */
	...
	const getCtx = <K extends keyof DisplayContext>(key: K): DisplayContext[K] => {
	...
	const format_currency = (value: unknown, currency: string, precision?: number): string => {
		const prec = typeof precision === "number" ? precision : getCtx("currency_precision");
		const formatter = getCtx("formatCurrencyPlain");
		const result = typeof formatter === "function" ? formatter(value, prec) : value;
